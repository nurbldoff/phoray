...
from phoray.element import Mirror, Detector
...
sg = Mirror(d=1/d, order=order, geometry=s, position=Vec(0.,0.,0.),
            rotation=Vec(0.,0.,0.), surface_offset=Vec(0.,0.,5.0))
...
det = Detector(surface=p, position=Vec(0, detx, dety),
              rotation=Vec(2*atan(dety/detx),0.,0.))
...
srcs = [GaussianSource(Vec(xsize, ysize, 0), divergence=Vec(xdisp, ydisp, 0),
