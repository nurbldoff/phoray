...
sg = ReflectiveGrating(d=1/d, order=order, geometry=s, # position=Vec(0.,0.,0.),
            rotation=Vec(90.,0.,0.))
...
det = Detector(geometry=p, position=Vec(0, dety, detx),
               rotation=Vec(90-degrees(2*atan(dety/detx)), 0., 0.))
...
srcs = [GaussianSource(position=Vec(0, row.source_y, row.source_x),
                       rotation=Vec(angle, 0, 0),
                       size=Vec(xslit, yslit, 0),
                       divergence=Vec(xdisp, ydisp, 0),
